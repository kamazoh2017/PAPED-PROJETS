...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [proRes, tRes, entRes] = await Promise.all([
          fetch('/api/projets'), fetch('/api/taches'), fetch('/api/entites'),
        ]);
  ...
  return (
    <div className="space-y-7" style={{ fontFamily: 'Sora, ui-sans-serif' }}>
      ...
      {/* Graphiques — ligne 4 */}
      <section className={`grid grid-cols-1 gap-4 lg:grid-cols-2 ${dashboardView !== 'taches' ? 'hidden' : ''}`}>
        {/* Tâche par ressource */}
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
